...
#include <aspect/material_model/glisovic_forte_fdr_aug21_compressible.h>
...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    void
    GlisovicForteFDRAug21Compressible<dim>::
    evaluate(const MaterialModel::MaterialModelInputs<dim> &in,
             MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      // outer loop through all parts of the model    
      for (unsigned int i=0; i < in.position.size(); ++i)
        {
          // extract depth at given position    
          const double depth = this->get_geometry_model().depth(in.position[i]);
          
          double thermal_viscosity_exponent= 0.;
            
          if (depth < 670000)
            {
              thermal_viscosity_exponent = thermal_viscosity_exponents[0];
            } 
          if (depth >= 670000)
            {
              thermal_viscosity_exponent = thermal_viscosity_exponents[1];
            }
          ...
          std::vector<double>  alpha_val (5,3.85e-5);
          std::vector<double>  depth_val_a (5,0);
          ...
          depth_val_a[2] = 2600000;
          depth_val_a[3] = 2890000;
	  
          if (depth < depth_val_a[1])
            {
	     ...
	     if (depth >= depth_val_a[2])
            {
	      alpha_val[2]=1.43e-5;	
	      B_val = (alpha_val[2] - alpha_val[3])/(depth_val_a[2] - depth_val_a[3]);
	      A_val = alpha_val[2] - B_val * depth_val_a[2];
	      thermal_alpha_val = A_val + B_val * depth;
            }
          ...
          double rhoP = reference_rho * std::exp(reference_compressibility * std::abs(in.pressure[i] - this->get_surface_pressure()));
          double rho = rhoP*(1 - thermal_alpha * (in.temperature[i]-reference_T)) + compositional_delta_rho*c*(rhoP/reference_rho);
		  out.densities[i]=rho; 
...
//    }
    ...
    template <int dim>
    void
    GlisovicForteFDRAug21Compressible<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Glisovic Forte FDR Aug21 Compressible model");
        {
          ...
          prm.declare_entry ("Reference compressibility", "4e-12",
                            Patterns::Double (0),
                            "The value of the constant reference compressibility $\\beta$. Units: $Pa^-1$.");
          ...
          prm.declare_entry ("Thermal viscosity exponents", "0.0",
                             ...
                             "List of temperature dependence of viscosity values. Dimensionless exponent. "
    ...
    template <int dim>
    void
    GlisovicForteFDRAug21Compressible<dim>::parse_parameters (ParameterHandler &prm)
    {
      // increment by one for background:
      const unsigned int n_fields = this->n_compositional_fields() + 1;
      const unsigned int n_depths = 2;
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Glisovic Forte FDR Aug21 Compressible model");
        {
          ...
          thermal_viscosity_exponents = Utilities::possibly_extend_from_1_to_N (Utilities::string_to_double(Utilities::split_string_list(prm.get("Thermal viscosity exponents"))),
                                                              n_depths,
                                                              "Thermal viscosity exponents");
          ...
          if (thermal_viscosity_exponents[0]!=0.0 && thermal_viscosity_exponents[1]!=0.0 && reference_T == 0.0)
      ...
      if (thermal_viscosity_exponents[0] != 0 || thermal_viscosity_exponents[1] != 0)
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(GlisovicForteFDRAug21Compressible,
                                   "Glisovic Forte FDR Aug21 Compressible",
                                   "")
  }
}
