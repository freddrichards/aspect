...
#include <aspect/material_model/glisovic_forte_fdr.h>
...
namespace aspect
{
  namespace MaterialModel
  {
             ...
             MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      for (unsigned int i=0; i < in.position.size(); ++i)
        {
          ...
          std::vector<double>  alpha_val (5,3.95e-5);
	  ...
	  depth_val_a[1] = 410000;
          depth_val_a[2] = 710000;
          depth_val_a[3] = 2600000;
	  depth_val_a[4] = 2890000;
	  ...
	  if (depth >= depth_val_a[3])
            {
              //comment out alpha_val[3] if using thermal only
	      //alpha_val[3]=-1.83e-5;	
	      B_val = (alpha_val[3] - alpha_val[4])/(depth_val_a[3] - depth_val_a[4]);
	      A_val = alpha_val[3] - B_val * depth_val_a[3];
	      thermal_alpha_val = A_val + B_val * depth;
            }
    ...
    template <int dim>
    void
    GlisovicForteFDR<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Glisovic Forte FDR model");
        {
    ...
    template <int dim>
    void
    GlisovicForteFDR<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Glisovic Forte FDR model");
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(GlisovicForteFDR,
                                   "Glisovic Forte FDR",
                                   "")
  }
}
